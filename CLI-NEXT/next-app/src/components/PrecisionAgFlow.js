...
export default function PrecisionAgFlow({ location }) {
    const [status, setStatus] = useState('DRAFT'); // DRAFT, DETAILS_COMPLETE, ANALYZING, RESULTS_READY
                ...
                <div className="animate-in fade-in duration-1000">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <div className="flex items-center gap-2 px-4 py-2 bg-[#00D09C]/10 rounded-xl border border-[#00D09C]/20">
                            ...
                            <span className="text-[10px] font-black text-[#00D09C] uppercase tracking-widest">Live Analysis Active</span>
                        </div>
                    </div>
                    <PrecisionAgDashboard
                        location={location}
                        cropType={fieldData?.cropType}
                        daysAfterSowing={45} // Can calculate from fieldData.sowingDate
                    />
                </div>
            )}
        </div>
