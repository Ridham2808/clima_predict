  ...
  return (
    <div className="min-h-screen text-white pb-32 md:pb-12 overflow-x-hidden selection:bg-[#00D09C]/30">
      <div className="w-full mx-auto">
        ...
        <header className="w-full px-6 pt-6 pb-4 md:px-2 md:pt-0 md:mb-8">
          ...
          <div className="flex bg-white/5 backdrop-blur-lg p-1.5 rounded-[1.5rem] border border-white/10">
            {['overview', 'forecast', 'stats'].map((tab) => (
              <button
                ...
                className={`flex-1 py-2.5 text-sm font-bold rounded-2xl capitalize transition-all duration-300 ${activeTab === tab
        ...
        <main className="w-full px-6 md:px-2">
            ...
            <div className="col-span-12 lg:col-span-4 space-y-8">
              <div className="bg-gradient-to-br from-[#00D09C] to-[#4D9FFF] rounded-[3rem] p-10 shadow-2xl relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  <div className="text-9xl font-black text-white mb-2 tracking-tighter drop-shadow-2xl">
                    {weatherData.temperature}°
                  </div>
                  <div className="text-3xl font-black text-white/90 uppercase tracking-widest mb-4">{weatherData.condition}</div>
                  <div className="inline-block px-5 py-2 bg-black/10 backdrop-blur-md rounded-full text-sm font-bold mb-10">
                  ...
                  <div className="grid grid-cols-2 gap-6 pt-8 border-t border-white/20">
                    <div className="flex flex-col">
                      ...
                      <span className="text-3xl font-black">{weatherData.humidity}%</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-white/60 text-[10px] uppercase font-black tracking-widest mb-1">Wind</span>
                      <span className="text-3xl font-black">{weatherData.windSpeed} <span className="text-sm text-white/60">km/h</span></span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* AQI Desktop Card */}
              {airQuality && (
                <div className="bg-white/5 border border-white/5 rounded-[2.5rem] p-8 relative overflow-hidden group hover:bg-white/[0.07] transition-colors">
                  <div className="flex justify-between items-start relative z-10">
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[#FFC857]">Atmospheric Quality</span>
                      <div className="text-7xl font-black mt-2 leading-none text-white">{airQuality.aqi}</div>
                      <div className="text-xl font-bold mt-4 text-[#FFC857]">{airQuality.category}</div>
                    </div>
                    <div className="p-5 bg-[#FFC857]/10 rounded-3xl border border-[#FFC857]/20">
                      <Wind width={40} height={40} className="text-[#FFC857]" />
                    </div>
                  </div>
                  <p className="mt-8 text-white/50 font-medium italic leading-relaxed">"{airQuality.recommendation}"</p>
                </div>
              )}
            </div>
              ...
              <div className="bg-white/5 border border-white/5 rounded-[2.5rem] p-8">
                ...
                <div className="space-y-4">
                  {forecastData.slice(0, 7).map((day, idx) => (
                    <div key={idx} className="flex items-center justify-between p-4 bg-white/[0.02] hover:bg-white/5 border border-white/5 rounded-2xl transition-all">
                      ...
                      <div className="flex items-center gap-8">
                        <div className="flex items-center gap-2">
                          <Droplet width={14} height={14} className="text-[#4D9FFF] opacity-60" />
                          <span className="text-xs font-black text-white/40">{day.precipitation}%</span>
                        </div>
              ...
              <div className="bg-[#151515] border border-white/5 rounded-[2.5rem] p-8">
                ...
                <div className="space-y-4">
                  {alerts.length > 0 ? alerts.slice(0, 3).map((alert, idx) => (
                    <div key={idx} className="p-5 bg-white/5 rounded-3xl border border-white/5 group hover:border-white/20 transition-all cursor-pointer">
                      <div className="flex items-center gap-4 mb-3">
                        <div className="p-2.5 rounded-xl" style={{ backgroundColor: `${alert.color}15`, color: alert.color }}>
                          {(() => {
                      ...
                      <p className="text-xs text-white/40 font-medium leading-relaxed line-clamp-2">{alert.description}</p>
                    </div>
                  )) : (
          ...
          <div className="md:hidden">
            {activeTab === 'overview' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <Link href="/weather-details" className="block mb-6 active:scale-[0.98] transition-all">
                  <div className="relative bg-gradient-to-br from-[#00D09C] via-[#00D09C] to-[#4D9FFF] rounded-[2.5rem] p-8 shadow-[0_20px_50px_rgba(0,208,156,0.3)] overflow-hidden group">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-10 -mt-10 blur-3xl opacity-50" />
                    <div className="absolute bottom-0 left-0 w-24 h-24 bg-black/10 rounded-full -ml-8 -mb-8 blur-2xl opacity-50" />

                    <div className="relative z-10 flex flex-col items-center">
                      <div className="text-8xl font-black text-white mb-2 tracking-tighter drop-shadow-2xl">
                        {weatherData.temperature}°
                      </div>
                      <div className="text-2xl font-bold text-white mb-1 uppercase tracking-widest opacity-90">{weatherData.condition}</div>
                      <div className="text-sm text-white/70 mb-8 font-medium bg-black/10 px-4 py-1.5 rounded-full backdrop-blur-sm">
                        Feels like {weatherData.feelsLike}°
                      </div>

                      <div className="grid grid-cols-3 gap-4 w-full pt-6 border-t border-white/20">
                        <div className="flex flex-col items-center">
                          <Droplet width={24} height={24} className="text-white mb-2 opacity-80" />
                        ...
                        <div className="flex flex-col items-center border-x border-white/10">
                          <Wind width={24} height={24} className="text-white mb-2 opacity-80" />
                        ...
                        <div className="flex flex-col items-center">
                          <Eye width={24} height={24} className="text-white mb-2 opacity-80" />
                ...
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      <Link key={idx} href={item.href} className="group bg-white/5 backdrop-blur-md rounded-3xl p-5 border border-white/5 active:scale-95 transition-all">
